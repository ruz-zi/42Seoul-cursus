...
/*   Updated: 2021/03/29 18:17:47 by dju              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_flag(const char **s, va_list *va, t_flag *f)
{
	...
	return (ft_strchr("cspndiuxXh%", **s) && (*s)++ ? (*(*s - 1)) : -1);
}
...
static int	print_short(char specifier, va_list *va, t_flag *f)
{
	if (specifier == 'd' || specifier == 'i')
		return (print_sint(va_arg(*va, short), f));
	else if (specifier == 'u')
		return (print_uint(va_arg(*va, unsigned short), f, B_DEC));
	else if (specifier == 'x')
		return (print_uint(va_arg(*va, unsigned short), f, B_LHEX));
	else if (specifier == 'X')
		return (print_uint(va_arg(*va, unsigned short), f, B_UHEX));
	return (0);
}
